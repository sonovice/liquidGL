 ...
 * -----------------------------------------------------------------------------
 * Requirements:   WebGL (falls back gracefully) + html2canvas
  ...
  class LiquidGlass {
    ...
    render() {
      ...
      if (this.uBounds && this.uvScale[0] > 0) {
        // This is the optimized path. We use the cached initial position and the
        // live scroll offset to calculate the texture coordinates without forcing
        // a costly DOM layout reflow on every frame.
        const docX = this.initialX; // X is constant for a fixed element
        const docY = this.initialY + this.scrollOffset;
